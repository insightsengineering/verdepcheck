...
#' @keywords internal
get_gh_releases <- function(org, repo) {
  gql_query <- sprintf("{
  ...
  non_prerelease <- Filter(function(x) isFALSE(x$isPrerelease), resp$data$repository$releases$nodes)
  vapply(non_prerelease, `[[`, character(1), "tagName")
}
#' @importFrom gh gh_gql
#' @keywords internal
